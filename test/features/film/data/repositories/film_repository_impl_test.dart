...
// ---------------------------------------------------------------------------
// Tests unitaires FilmRepositoryImpl
// ---------------------------------------------------------------------------
void main() {
  ...
  group('FilmRepositoryImpl - getAllFilm', () {
    test(
      '1. Succès réseau : retourne les films distants et les sauvegarde en cache local SQLite',
      () async {
    ...
    test(
      '2. Échec réseau (mode hors-ligne) : retourne les données du cache local SQLite',
      () async {
  ...
  group('FilmRepositoryImpl - getFilmById', () {
    ...
    test(
      '5. Échec réseau : retourne le film depuis le cache local SQLite',
      () async {
        final film = sampleFilms.first;
