...
// ---------------------------------------------------------------------------
// Tests unitaires CreditsRepositoryImpl
// ---------------------------------------------------------------------------
void main() {
  ...
  group('CreditsRepositoryImpl - findByFilmId', () {
    test(
      '1. Succès réseau : retourne les crédits et les enregistre en cache local SQLite',
      () async {
    ...
    test(
      '2. Échec réseau (mode hors-ligne) : retourne les crédits du cache local SQLite',
      () async {
    ...
    test(
      '3. Échec réseau sans cache local : lève une exception réseau mappée',
      () async {
  ...
  group('CreditsRepositoryImpl - findByPersonId', () {
    test(
      '4. Succès réseau : retourne la filmographie et l\'enregistre en cache local',
      () async {
        fakeRemote.creditsToReturn = sampleCredits;

        final result = await repository.findByPersonId(20);
    ...
    test(
      '5. Échec réseau (mode hors-ligne) : retourne la filmographie du cache SQLite',
      () async {
        fakeLocal.cachedCredits = sampleCredits;
        fakeRemote.exceptionToThrow = DioException(
          requestOptions: RequestOptions(path: '/credits'),
